...
public final class Registrations2Epics {
  ...
  static Topology createTopology(Properties props) {
    ...
    final KStream<String, String> output =
        input.process(new MyProcessorSupplier(storeBuilder.name()), storeBuilder.name());
  ...
  /**
   * Factory to create Kafka Streams Transformer instances; references a stateStore to maintain
  ...
  private static final class MyProcessorSupplier
        ...
        public void process(Record<String, Alarm> input) {
          ...
          if (input.value()
            ...
            if (previous != null) { // We only store EPICSProducer, so no need to check type
              ...
              output =
                  new Record<>(
                      toJsonKey(channel), toJsonValue(input.key(), input.value()), timestamp);
            ...
            store.put(
                input.key(),
                input.value()); // Store most recent non-null registration for each CA alarm (key)
          }
    ...
    private void populateHeaders(Record<String, String> record) {
      ...
      record
          .headers()
          .add("user", System.getProperty("user.name").getBytes(StandardCharsets.UTF_8));
